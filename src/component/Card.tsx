...
const Card = (props) => {
  ...
  const CardCode = (
    <div
      className="
      CardContainer m-0  border rounded-lg  bg-gray-800
      border-gray-700 border-0 "
      onClick={props.onClick}
    >
      <div className="CardImageContainer rounded-lg overflow-hidden relative w-full min-h-fit block">
        < a href="#" >
          <img
            className={`CardImage  transform transition-transform
            duration-300 ease-out hover:scale-125 hover:shadow-blue-2 w-full
            object-cover object-center h-full ${props.imgStyle} `}
            src={props.imgUrl}
            alt={props.title}
          />
          <div class="Play h-6 w-6 ">
